...
public class DapperRatingRepository : IRatingRepository
{
    ...
    public DapperRatingRepository(IDbConnectionFactory dbConnectionFactory)
    {
    ...
    public async Task<float?> GetAverageMovieRatingAsync(Guid movieId, CancellationToken token = default)
    {
        using var connection = await _dbConnectionFactory.CreateConnectionAsync(token);
        return await connection.QuerySingleOrDefaultAsync<float?>(new CommandDefinition("""
            select round(avg(r.rating), 1) as AverageRating from ratings r
            where movieid = @movieId
            """, new { movieId }, cancellationToken: token));
    }

    public async Task<(float? AverageRating, int? UserRating)> GetAverageMovieRatingByUserAsync(Guid movieId, Guid userId, CancellationToken token = default)
    {
        using var connection = await _dbConnectionFactory.CreateConnectionAsync(token);
